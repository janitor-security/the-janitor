...
# ${owner}/${repo_name} PR Audit Report — Janitor v${JANITOR_VERSION}

**Date**: ${today}
**PRs Audited**: ${total_prs}
**Engine**: The Janitor v${JANITOR_VERSION} (tree-sitter AST, MinHash LSH, ML-DSA-65 attestation)
...
The Janitor v${JANITOR_VERSION} runs a 6-stage structural analysis pipeline on each pull request diff:
...
*Generated by The Janitor v${JANITOR_VERSION} — <https://thejanitor.app>*
MARKDOWN
...
_synthesize_vex() {
    ...
    if [[ "${security_threat_count}" -eq 0 ]]; then
        ...
        vex_detail="Janitor v${JANITOR_VERSION} structural scan found 0 security: antipattern findings across all audited PRs. All repository components assessed as not_affected."
    else
        vex_state="affected"
        has_justification="false"
        vex_detail="Janitor v${JANITOR_VERSION} structural scan found ${security_threat_count} PR(s) with security: antipattern findings. Review gauntlet_intelligence_report.pdf and gauntlet_export.csv for remediation detail."
    fi
    ...
    jq -n \
        ...
        --arg  janitor_version "${JANITOR_VERSION}" \
        '{
          bomFormat:    "CycloneDX",
          specVersion:  "1.5",
          version:      1,
          ...
          vulnerabilities: [{
            ...
            description: (
              ("Static structural security analysis by The Janitor v" + $janitor_version + ". ") +
