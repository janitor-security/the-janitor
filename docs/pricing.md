...
### Lead Specialist — $4,900 / year

**A professional digital license for engineering teams that need to prove code integrity, not just perform cleanup.**

Includes all Junior Janitor capabilities, plus the following license-gated features:

| Capability | Included |
|:-----------|:--------:|
| **PQC-Signed Audit Logs** — every cleanup event automatically signed with Ed25519, stored in `.janitor/audit_log.json` | ✓ |
| **Sovereign Status Badges** — tamper-evident SVG badge with embedded signature, automatically generated by the tool | ✓ |
| **CI/CD Compliance Attestation** — `--token` flag activates signed reports in GitHub Actions, GitLab CI, Jenkins | ✓ |
| **License Issue SLA** — 48-hour response for license key delivery and renewal issues | ✓ |
| Up to 10 named seats on a single license | ✓ |
...
### Industrial Core — Custom (from $50,000 / year)

**An enterprise digital license for organizations where code integrity is a compliance obligation, not a preference.**

Includes all Lead Specialist capabilities, plus:

- **On-Premises Token Server** — the binary is issued with a dedicated verifying key for air-gapped deployments; zero external calls required
- **Keypair Rotation Protocol** — enterprise licensees receive a signed rotation schedule; invalidating a compromised token requires only a keypair rotation, not a binary update, satisfying SOC 2 Type II change-management requirements
- **Enterprise License SLA** — contractual response times for license key issuance, renewal, and emergency rotation
- **Extended CI/CD Compatibility** — the binary operates with Bazel, Pants, Meson, and internal CI pipelines without modification
- **Unlimited named seats** on a single organization license
- **Dedicated onboarding** for configuration review and CI/CD pipeline integration
