...
**A digital license for organizations that need to prove, not just perform.**
...
| Up to 5 named seats on a single license | ✓ |
...
**An enterprise digital license for codebases exceeding 1 million LOC.**

Includes all Lead Specialist capabilities, plus:

- **On-Premises Token Server** — the binary is issued with a private verifying key for air-gapped deployments; no external calls ever required
- **License SLA** — documented response times for license key issuance, renewal, and rotation
...
- **Volume pricing** for multi-repo or multi-team deployments
